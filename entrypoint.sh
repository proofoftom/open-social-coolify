...
mysql -h "${DB_HOST:-mariadb}" -P "${DB_PORT:-3306}" -u "root" -p"${DB_ROOT_PASSWORD:-rootpassword}" --skip-ssl -e
"CREATE DATABASE IF NOT EXISTS \`${DB_NAME:-opensocial}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;"
...
IFS=',' read -ra PATTERNS <<< "$DRUPAL_TRUSTED_HOST_PATTERNS" for pattern in "${PATTERNS[@]}" ; do
    TRUSTED_HOSTS="$TRUSTED_HOSTS  '$pattern',\n" done fi # Add Coolify-generated hostnames to trusted hosts if [
    -n "$COOLIFY_URL" ]; then # Extract hostname from URL COOLIFY_HOST=$(echo "$COOLIFY_URL" | sed -e 's|^[^/]*//||'
    -e 's|/.*$||' -e 's|:.*$||' ) COOLIFY_PATTERN=$(echo "$COOLIFY_HOST" | sed 's/\./\\\\./g' )
    TRUSTED_HOSTS="$TRUSTED_HOSTS  '^${COOLIFY_PATTERN}\$',\n" fi # Always allow localhost
    TRUSTED_HOSTS="$TRUSTED_HOSTS  '^localhost\$',\n" TRUSTED_HOSTS="$TRUSTED_HOSTS  '^127\\\\.0\\\\.0\\\\.1\$',\n" #
    Append database and other settings to settings.php if not already configured if ! grep
    -q "Added by entrypoint" "$SETTINGS_FILE" 2>/dev/null; then
    echo "Configuring settings.php..."
    cat >> "$SETTINGS_FILE" << SETTINGS // Added by entrypoint \$databases['default']['default']=[ 'database'=>
        '${DB_NAME:-opensocial}',
        ...
        $DRUSH social-demo:add file user group topic event event_enrollment comment post like || echo "Failed to add
        demo content"
        ...
        echo "Copying custom modules to Drupal modules directory ($CUSTOM_MODULES_DEST)..."
        ...
        $DRUSH config:set siwe_login.settings expected_domain "$SIWE_DOMAIN" -y || echo "Failed to configure SIWE Login
        domain"
        else
