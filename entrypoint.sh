...
IFS=',' read -ra PATTERNS <<< "$DRUPAL_TRUSTED_HOST_PATTERNS" for pattern in "${PATTERNS[@]}" ; do
    ...
    cat >> "$SETTINGS_FILE" << SETTINGS // Added by entrypoint \$databases['default']['default']=[ 'database'=>
        ...
        $DRUSH en social_demo -y || echo "Failed to enable Social Demo module"
        $DRUSH cr
        $DRUSH social-demo:add file user group topic event event_enrollment comment post like || echo "Failed to create
        demo content"
