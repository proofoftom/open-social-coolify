...
2. **PostgreSQL Database** - Stores all Open Social data

## Key Components

### Dockerfile
The Dockerfile creates a custom PHP 8.3 Apache image with all the necessary dependencies for running Open Social:
- PHP extensions: PDO, PostgreSQL, GD, MBString, EXIF, PCNTL, BCMath, ZIP, Opcache, Intl
...
- Verify database connection using `nc -zv postgres 5432` from the app container
